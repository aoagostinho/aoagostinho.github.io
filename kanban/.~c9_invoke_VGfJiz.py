    ...
    if request.method == "GET":
        ...
        if not request.form.get("symbol"):
            ...
            if symbol == None:
                ...
                purchase = symbol["price"] * shares
                if current_money < purchase:
                    return apology("You don't have enough cash", "No cash")
                else:
                    check_unique = db.execute("SELECT * FROM shares_index WHERE usr_id =:logged AND symbol=:symbol", logged = log_usr, symbol = symbol["symbol"])
                    if not check_unique:
                        db.execute("INSERT INTO shares_index (usr_id, symbol, name, shares) VALUES (:logged, :symbol, :name, :shares)",
                        logged = log_usr, symbol = symbol["symbol"], name = symbol["name"], shares = shares)
                        db.execute("UPDATE users SET cash = cash - :purchase WHERE id = :logged", purchase = purchase, logged = log_usr)
                        db.execute("INSERT INTO transactions (usr_id, usr_name, symbol, name, shares, unit_price, transacted) VALUES (:logged, ")
                        return apology("SUCCESS")
                    else:
                        db.execute("UPDATE shares_index SET shares  = shares + :newshares WHERE usr_id = :logged AND symbol = :symbol",
                        newshares = shares, logged = log_usr, symbol = symbol["symbol"])
                        db.execute("UPDATE users SET cash = cash - :purchase WHERE id = :logged", purchase = purchase, logged = log_usr)
                        return apology("SUCCESS")
